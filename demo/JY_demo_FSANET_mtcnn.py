...
def draw_axis(img, yaw, pitch, roll, tdx=None, tdy=None, size = 80):
    ...
    pitch_show = 'Pitch :',round(pitch,2) 
    roll_show = 'Roll :' , round(roll,2)
    yaw_show = 'yaw :' , round(yaw,2) 

    cv2.line(img, (int(tdx), int(tdy)), (int(x1),int(y1)),(0,0,255),3)
    cv2.line(img, (int(tdx), int(tdy)), (int(x2),int(y2)),(0,255,0),3)
    cv2.line(img, (int(tdx), int(tdy)), (int(x3),int(y3)),(255,0,0),2)
  
    cv2.putText(img, str(pitch_show)  ,(0, 10), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (0, 255, 0), 1)
    cv2.putText(img, str(yaw_show) , (0, 30), cv2.FONT_HERSHEY_SIMPLEX,  0.3, (0, 255, 0), 1)
    cv2.putText(img, str(roll_show) , (0, 50), cv2.FONT_HERSHEY_SIMPLEX , 0.3, (0, 255, 0),1)
